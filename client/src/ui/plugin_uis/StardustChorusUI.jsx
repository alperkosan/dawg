  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (inputLevel > 0.1 && particlesRef.current.length < 100) {
        for (let i = 0; i < 3; i++) {
          particlesRef.current.push(new ChorusParticle(width/2 + (Math.random() - 0.5) * 100, height/2 + (Math.random() - 0.5) * 100));
        }
      }
...
export const StardustChorusUI = ({ trackId, effect, onChange }) => {
  ...
  useEffect(() => {
    ...
    // === DÜZELTME BURADA: Gelen 'data' objesinden 'peak' özelliğini okuyoruz ===
  ...
  return (
    <div className="w-full h-full bg-gradient-to-br from-indigo-950 via-purple-950 to-pink-950 p-6">
          ...
          <div className="grid grid-cols-3 gap-1">
            {['◦', '◉', '●'].map((pattern, i) => (
              <button key={i} onClick={() => onChange('depth', (i + 1) * 0.3)}
                className={`w-8 h-8 rounded-full border transition-all ${ Math.abs(depth - (i + 1) * 0.3) < 0.1 ? 'border-purple-400 bg-purple-500/30' : 'border-purple-600/50 hover:border-purple-400/70' }`}
              >
      ...
      <div className="grid grid-cols-4 gap-6">
        <ProfessionalKnob label="Rate" value={frequency} onChange={(v) => onChange('frequency', v)} min={0.1} max={10} defaultValue={1.5} unit="Hz" precision={2} size={75} />
        <ProfessionalKnob label="Delay" value={delayTime} onChange={(v) => onChange('delayTime', v)} min={1} max={20} defaultValue={3.5} unit="ms" precision={1} size={75} />
        <ProfessionalKnob label="Depth" value={depth * 100} onChange={(v) => onChange('depth', v / 100)} min={0} max={100} defaultValue={70} unit="%" precision={0} size={75} />
        <ProfessionalKnob label="Mix" value={wet * 100} onChange={(v) => onChange('wet', v / 100)} min={0} max={100} defaultValue={50} unit="%" precision={0} size={75} />
      </div>
