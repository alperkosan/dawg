...
export class NativeAudioEngine {
    ...
    /**
     * Set mixer insert solo state
     * @param {string} insertId - Insert ID
     * @param {boolean} isSolo - Solo state
     */
    setSolo(insertId, isSolo) {
        // Initialize solo set if not exists
        if (!this.soloedInserts) {
            this.soloedInserts = new Set();
        }

        // Update solo state set
        if (isSolo) {
            this.soloedInserts.add(insertId);
        } else {
            this.soloedInserts.delete(insertId);
        }

        const isAnySoloed = this.soloedInserts.size > 0;

        // Apply solo logic to ALL inserts
        this.mixerInserts.forEach((targetInsert, targetId) => {
            // Master channel is never muted by solo
            if (targetId === 'master') return;

            // If we have soloed channels in the system:
            if (isAnySoloed) {
                // If THIS target is one of the soloed ones:
                if (this.soloedInserts.has(targetId)) {
                    // It should play (Solo Mute = false)
                    if (targetInsert.setSoloMute) targetInsert.setSoloMute(false);
                } else {
                    // It should be silent (Solo Mute = true)
                    if (targetInsert.setSoloMute) targetInsert.setSoloMute(true);
                }
            } else {
                // No solo active anywhere:
                // Everything should be audible (Solo Mute = false)
                // (User mute state typically handles the rest)
                if (targetInsert.setSoloMute) targetInsert.setSoloMute(false);
            }
        });

        logger.info(NAMESPACES.AUDIO, `Set solo for ${insertId}: ${isSolo} (Total soloed: ${this.soloedInserts.size})`);
