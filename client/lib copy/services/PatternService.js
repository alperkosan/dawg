...
import { useArrangementStore } from '../../src/store/useArrangementStore';
...
export class PatternService {
  ...
  static clearPattern(patternId) {
    ...
    if (engine) {
      const patternData = new PatternData(pattern.id, pattern.name, clearedData);
  ...
  static quantizePattern(patternId, gridValue = '16n', strength = 1.0) {
    ...
    const engine = AudioContextService.getAudioEngine();
    if (engine) {
      const patternData = new PatternData(pattern.id, pattern.name, quantizedData);
      engine.patterns.set(patternId, patternData);
    }
  ...
  static optimizePattern(patternId) {
    ...
    const engine = AudioContextService.getAudioEngine();
    if (engine) {
      const patternData = new PatternData(pattern.id, pattern.name, optimizedData);
      engine.patterns.set(patternId, patternData);
    }
  ...
  static mergePatterns(patternIds, newName = 'Merged Pattern', mergeMode = 'sequence') {
    ...
    const engine = AudioContextService.getAudioEngine();
    if (engine) {
      const patternData = new PatternData(mergedPattern.id, mergedPattern.name, mergedPattern.data);
      engine.patterns.set(mergedId, patternData);
    }
  ...
  static importPattern(patternData, name = 'Imported Pattern') {
    try {
      ...
      if (patternData.data) {
        ...
        const engine = AudioContextService.getAudioEngine();
        if (engine) {
          const enginePattern = new PatternData(
            importedPattern.id, 
            importedPattern.name, 
            patternData.data
          );
          engine.patterns.set(importedPattern.id, enginePattern);
        }
      }
  ...
  static validateAndFixPattern(patternId) {
    ...
    if (wasFixed) {
      ...
      const engine = AudioContextService.getAudioEngine();
      if (engine) {
        const patternData = new PatternData(pattern.id, pattern.name, fixedData);
        engine.patterns.set(patternId, patternData);
      }
    }
  ...
  static updateNotesForActivePattern(instrumentId, newNotes, shouldReschedule = true) {
    ...
    if (engine) {
      ...
      if (pattern) {
        ...
        engine.patterns.set(activePatternId, patternData);
      }
    }
    ...
    if (playbackStore.isAutoLoop) {
      playbackStore.updateLoopSettings();
    }
  ...
  static createNewPattern(name = 'New Pattern', length = 64) {
    ...
    const engine = AudioContextService.getAudioEngine();
    if (engine) {
      const patternData = new PatternData(newPattern.id, newPattern.name, newPattern.data);
      engine.patterns.set(patternId, patternData);
    }
  ...
  static duplicatePattern(sourcePatternId, newName = null) {
    ...
    const engine = AudioContextService.getAudioEngine();
    if (engine) {
      const patternData = new PatternData(duplicatedPattern.id, duplicatedPattern.name, duplicatedPattern.data);
      engine.patterns.set(duplicatedId, patternData);
    }
  ...
  static deletePattern(patternId) {
    ...
    const engine = AudioContextService.getAudioEngine();
    if (engine) {
      engine.patterns.delete(patternId);
    }
