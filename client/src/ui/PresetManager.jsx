...
export const PresetManager = ({ pluginType, effect, factoryPresets = [], onChange }) => {
  ...
  const handlePresetSelect = (presetSettings, presetName) => {
  ...
  return (
    <div className="absolute top-0 left-0 right-0 h-10 bg-gray-900/50 flex items-center justify-between px-3 z-10 border-b border-black/20">
                ...
                {factoryPresets.length > 0 && (
                    <>
                        ...
                        {factoryPresets.map(p => 
                            <button key={p.name} onClick={() => handlePresetSelect(p.settings, p.name)} className="w-full text-left text-sm px-2 py-1.5 rounded hover:bg-cyan-700">{p.name}</button>
                        )}
                    </>
                )}
                    ...
                    <>
                        ...
                        {userPresets.map(p => 
                            <div key={p.name} className="group flex items-center justify-between w-full text-left text-sm rounded hover:bg-cyan-700">
                                <button onClick={() => handlePresetSelect(p.settings, p.name)} className="flex-grow text-left px-2 py-1.5">{p.name}</button>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-1 bg-gray-800 rounded-md p-0.5">
            <button 
                ...
                className={`px-2 py-0.5 text-xs font-bold rounded ${effect.abState?.isB ? 'bg-gray-600' : 'bg-amber-500 text-gray-900'}`}
            >A</button>
             <button 
                onClick={handleToggleAB}
