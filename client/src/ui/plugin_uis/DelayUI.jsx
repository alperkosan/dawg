...
export const DelayUI = ({ effect, onChange }) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (Math.random() < 0.05) echoes.push({ pos: 0, life: 1 });
      ...
      echoes.forEach((echo, i) => {
        ...
        echo.pos += (1 / (delayInSeconds + 0.01)) * 0.016;
        echo.life *= (0.99 + feedback * 0.005);
        if (echo.pos > 1 || echo.life < 0.01) { echoes.splice(i, 1); return; }
  ...
  return (
    <div className="plugin-content-layout delay-ui">
      <ProfessionalKnob label="Feedback" value={effect.settings.feedback * 100} onChange={(val) => onChange('feedback', val / 100)} min={0} max={95} defaultValue={30} unit="%" precision={0} size={90} />
      ...
      <ProfessionalKnob label="Mix" value={effect.settings.wet * 100} onChange={(val) => onChange('wet', val / 100)} min={0} max={100} defaultValue={35} unit="%" precision={0} size={90} />
    </div>
  );
};
