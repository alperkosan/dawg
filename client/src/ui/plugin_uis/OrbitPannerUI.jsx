...
export const OrbitPannerUI = ({ effect, onChange, definition }) => {
  return (
    <div className="relative w-full h-full p-4 bg-gray-800 rounded-lg flex flex-col items-center justify-between border border-gray-700
                    bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-purple-900/40 to-gray-800">
    <PresetManager 
    ...
    />
        <div>
            <h3 className="text-lg font-bold text-purple-300">{definition.type}</h3>
        ...
        <div className="flex items-end justify-around w-full mt-4 gap-6">
            <VolumeKnob 
                ...
                min={0.1} max={10} defaultValue={2} size={72}
            />
            <VolumeKnob 
                label="Depth"
                value={effect.settings.depth}
                onChange={(val) => onChange('depth', val)}
                min={0} max={1} defaultValue={1} size={54}
            />
        </div>
    </div>
  );
